"""Step-by-step walkthrough of the wiki sequence diagram.

Where the ``demo`` scenarios narrate at the "is the property preserved?"
level, the walkthrough mirrors the wiki's architecture-page sequence
diagrams step-by-step, printing the actual JSON-RPC / SSE / OAuth
envelopes at each hop. Use it to:

  - Demonstrate the bridge end-to-end without standing up a server
  - Read the architecture page side-by-side with executable output
...
import bridge.commands  # noqa: F401 — register task-tracker commands
...
def walkthrough_a2a(*, tier: int, pause: bool) -> int:
    """End-to-end happy path on the A2A surface, mirroring the wiki sequence."""

    print(f"{_BOLD}A2A → Vault → RS walkthrough (Tier {tier}){_RESET}")
    print(f"{_DIM}A reference simulation of the sequence diagram in "
          f"`decisions/a2a-mcp-bridge-architecture` §A2A flow.{_RESET}")
    ...
    s.step("MCP host → Bridge",
           "POST /a2a  message:send (resume with signed payload)")
    ...
    print(f"\n{_GREEN}{_BOLD}Walkthrough complete.{_RESET}  All steps proceeded as the wiki sequence describes.")
    return 0
