...
- **Single-use that survives a restart and spans replicas**: `bridge/vault/durable_state.py` backs both decisions - consumed signatures at mint, consumed `jti`s at consume - with a shared SQLite file whose claim is an atomic `INSERT OR IGNORE`, so two replicas racing to present the same payload cannot both win. Both Vaults and the RS take it as an optional `durable_state=` kwarg and default to the in-memory sets. See "Single process vs. horizontal scale" below for why this becomes load-bearing under a stateless transport.
...
│   └── durable_state.py ← shared, TTL-aware replay store (restart + cross-replica)
...
## Single process vs. horizontal scale

Every guarantee above is stated per-process. Most of them stay true when you run more than one process, because they are properties of bytes: a signature either verifies or it doesn't, a JWT's `authorization_details` either match the live request or they don't, and a second replica reaches the same verdict as the first without consulting anything.
...
So storage locality is a security boundary here, not a performance choice. The `bridge/vault/durable_state.py` module is the seam: pass one `DurableReplayState` to every Vault and RS in the deployment and the single-use decision moves from N private sets to one atomic `INSERT OR IGNORE` against a shared file. The kwarg is optional and defaults to `None`. That keeps the single-process demo dependency-free, and it also means a multi-replica deployment that omits the shared state silently retains the vulnerability. Sizing, purge, and the shared-storage locking requirement are covered in "Known production gaps" below.
...
- **Durable storage substrate.** The token store is a JSON file with non-atomic load-modify-save. The audit log is SQLite without field-level encryption or scrubbing. Production swaps in durable, transactional, encrypted storage as a substrate change with no architectural impact. (Replay state is the exception and is no longer a gap: `DurableReplayState` gives the consumed-signature and consumed-`jti` decisions a shared, atomic substrate, because unlike the token store those two are load-bearing for a security property rather than for convenience.)
...
- ~~**In-memory consumed-jti set.**~~ *Closed, by injection:* `bridge/vault/durable_state.py` provides a shared, TTL-aware SQLite store, and `OAuthVault`, `InProcessVault` and `JwtResourceServer` all accept it as an optional `durable_state=` kwarg. Pointed at one file, it closes both the restart window and the cross-replica window, and the claim is an atomic `INSERT OR IGNORE` rather than a check-then-set. Exercised by `tests/unit/test_durable_state.py`, including a real second OS process re-presenting a recorded approval. *Residual, and the reason this reads "closed by injection" rather than "closed":* the kwarg defaults to `None`, so a deployment that never passes it keeps the in-memory behaviour and the gap; the file must sit on locking-backed shared storage (WAL over NFS/CIFS without working locks is untested and could silently double-allow); and `purge_expired()` is never called automatically, so an operator must schedule it or the table grows without bound. Tier 1's cross-replica guarantee is at *mint* only - its `_issued` record stays process-local, so a Tier-1 credential presented to another replica fails as `SignatureMismatch` rather than as a replay, and the cross-replica *consume* guarantee is delivered at the RS.
